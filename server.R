...
# Include Health damages, Ecosystem services, and yield of palm oil 
    ...
    opt_V <- function (U, npar = 1, print = TRUE) {
      for (i in 2:length(time)){
        ...
        R[i] = palmoil_price*X[i]                                # Revenue function for developed land
        C[i] = (p_c*U[i])^c_coeff                    # Cost function using effort to develop land
        D[i] = (p_d*X[i])^d_coeff 
        V[i] = R[i]  - C[i] - D[i]                  # Value function
        V[length(time)] = ((R[length(time)] - C[length(time)] - D[length(time)] )*exp(-rho *(time[length(time)]-1)))/rho
      }
