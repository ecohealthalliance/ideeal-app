...
ESTV1 = (Bsocial/(((exp(b) - exp(b*(Xprivate/100)))/(exp(b)-1))))/(100*HA_in_1_percent)
##############################################
...
opt_W3 <- function (U, npar = 1) {
  for (i in 1:(length(time)-1)){
    ...
    profits[i] = R[i] - C[i]
    #disc_profits[i] = profits[i]*exp(-rho*(i-1)) 
    B[i] = ESTV1*((exp(b) - exp(b*(X[i]/100)))/(exp(b)-1))
    #disc_B[i] = B[i]*exp(-rho*(i-1)) 
    #D[i] = d1*Z[i]^d2
    #D[i] = d3*(exp(d4 * ((Z[i]*n1)^(1/2) - d5)) + d6)**(-1)
    #disc_D[i] = D[i]*exp(-rho*(i-1))
    W[i] = profits[i]  + B[i]
    W[length(time)] = ((profits[length(time)] + B[length(time)] )*exp(-rho *(time[length(time)]-1)))/rho
  }
  W_dis = W*exp(-rho*time) # Discounted value function
  NPV_W <- sum(W_dis)
  return(NPV_W)
