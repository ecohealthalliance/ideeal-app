...
ui <- dashboardPage(# skin = "green",
  header <- dashboardHeader(color = "green", inverted = TRUE,
                            title = "IDEEAL", dropdownMenuOutput("messageMenu")),
...
# Landing Page tab --------------------------------------------------------
      tabItem(tabName = "Picture",
              ...
              fluidRow(  
                box(
                  img(src = "img1.png", height = 400, width = 600), width=12)
                      ),
              fluidRow(  
                box(
...
# Key variable input box --------------------------------------------------
              ...
              shiny::helpText("The core palm oil yield is the amount of palm oil in metric tons produced
              ...
              sliderInput('expenditures', 'Total expenditures on prevention and control in the region (US$):', 9e6,
                          min = 0, max = 1e8, pre = "$"),
              # Maybe bold these?
              h2(textOutput("trial_text")),
              h2(textOutput("text8")), 
            "Increasing the value of ecosystem services will reduce the land conversion 
                  for the social optimal"
              )
              ), 
# Key plot output ---------------------------------------------------------
...
#### Difference in Effort between private and social optimums ####  
  ...
  output$trial_text <- renderText({ 
    paste("Sum Total Ecosystem Services Value ($US/ha):", "trial text") 
  })
